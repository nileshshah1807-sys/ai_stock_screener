...
class ReverseDCFModel:
    ...
    def analyze_row(self, row):
        ...
        if market_cap is None or market_cap <= 0:
            return self._empty_result("missing_market_cap", fcf, fcf_source, market_cap, revenue)
        if fcf is None or fcf <= 0:
            return self._empty_result("missing_or_negative_fcf", fcf, fcf_source, market_cap, revenue)
        ...
        fixed_growth = float(self.config.REVERSE_DCF_BASE_GROWTH)
        ...
        upside = ((base_case_value / market_cap) - 1) if base_case_value and market_cap > 0 else None
        fcf_yield = fcf / market_cap if market_cap > 0 else None
        revenue_fcf_margin = fcf / revenue if revenue and revenue > 0 else None

        return {
            "DCF_Status": "OK" if implied_fcf_growth is not None else "growth_above_model_range",
            ...
            "DCF_Base_Case_Upside": round(upside, 4) if upside is not None else np.nan,
            "DCF_Assessment": self._assessment(implied_fcf_growth, implied_terminal_growth),
        }

    @staticmethod
    def _empty_result(status, fcf, fcf_source, market_cap, revenue):
        return {
            "DCF_Status": status,
            "DCF_FCF_Source": fcf_source,
    ...
    @staticmethod
    def _assessment(implied_fcf_growth, implied_terminal_growth):
        ...
        if implied_fcf_growth <= 0.12 and terminal_ok:
            return "Reasonable"
        if implied_fcf_growth <= 0.22:
            return "Demanding"
        return "Stretched"
...
<table><tr><th>Rank</th><th>Symbol</th><th>Market Cap</th><th>FCF Yield</th><th>Implied 5Y FCF CAGR</th><th>Implied Terminal Growth</th><th>Base Case Upside</th><th>Assessment</th></tr>
{dcf_rows_html}
</table>
<div style="font-size:12px;color:#777;margin-top:8px;">Reverse DCF solves the market-implied assumptions behind today's market cap using the configured 5-year DCF model.</div>
</div>
...
<p>Model uses a 5-year explicit forecast, {fmt_pct(self.config.REVERSE_DCF_DISCOUNT_RATE)} discount rate, {fmt_pct(self.config.REVERSE_DCF_TERMINAL_GROWTH)} terminal growth for implied FCF CAGR, and {fmt_pct(self.config.REVERSE_DCF_BASE_GROWTH)} explicit growth for implied terminal growth.</p>
<table><tr><th>Rank</th><th>Symbol</th><th>Market Cap</th><th>Base FCF</th><th>FCF Yield</th><th>Implied 5Y FCF CAGR</th><th>Implied Terminal Growth</th><th>Base Case Upside</th><th>Assessment</th></tr>
{dcf_rows}
</table></div>
